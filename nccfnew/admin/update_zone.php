...
include '../process.php';
	if(isset($_GET['edit_id']) && !empty($_GET['edit_id'])){
            $id= $_GET['edit_id'];
            $stmt_edit = $connect->prepare('SELECT * FROM zones WHERE id=:uid');
	...
	if(isset($_POST['update_zone'])){
        ...
        if(in_array($ext, $allowes_ext)){
            	unlink($upload_dir.$edit_row['imgCOR']);
            	unlink($upload_dir.$edit_row['imgSEC']);
                move_uploaded_file($tmp_dir, $upload_dir.$picProfile);
                move_uploaded_file($tmp_dir2, $upload_dir.$picProfile2);
                
                $stmt=$connect->prepare('UPDATE zones SET name=:uname,address=:uaddress,phone=:uphone,nameCOR=:unameCOR,nameSEC=:unameSEC,imgCOR=:uimgCOR,imgSEC=:uimgSEC WHERE id=:uid');
...
<body>
	...
	<main>
		...
		<section id="admin-panel">
	    	<section id="update-zone-wrapper" class="update-section">
				<h5>Update Zone</h5>
				<form method="post" enctype="multipart/form-data">
					...
					if(isset($_GET['edit_id']) && !empty($_GET['edit_id'])){
            			...
            			$stmt_edit = $connect->prepare('SELECT * FROM zones WHERE id=:uid');
           				$stmt_edit->execute(array(':uid' => $id));
            			while($edit_row=$stmt_edit->fetch(PDO::FETCH_ASSOC)){
							extract($edit_row); 
            ?>
            
				<div class="row">
					<p class="element-title">SELECTED ZONE <?php echo $name?></p>
		        	<div class="input-field col s10">
		        		<input id="input_text" value="<?php echo $name?>" type="text" name="Zname">
		        	</div>
				</div>
		    	<div class="row">
				...
				<div class="row">
		        	...
		        	<div class="input-field col s10">
		        		<input id="input_text" value="<?php echo $nameCOR?>" type="text" name="CORname">
		        	</div>
				</div>
		    	<div class="row">
				...
				<div class="row">
		        	...
		        	<div class="input-field col s10">
		        		<input id="input_text" type="text" value="<?php echo $nameSEC?>" name="SECname">
		        	</div>
				</div>
				<div class="row">
					<p class="element-title">Family House Address</p>
		        	<div class="input-field col s10">
		        		<input id="input_text" type="text" value="<?php echo $address?>" name="Faddress">
		        	</div>
				</div>
				<div class="row">
					<p class="element-title">Official Lines</p>
		        	<div class="input-field col s10">
		        		<input id="input_text" type="text" value="<?php echo $phone?>" placeholder="Use a comma to seperate numbers" name="phone">
		        	</div>
				</div>
				<button class="btn waves-effect waves-light" type="submit" name="update_zone">Add Zone</button>
				<?php
				}
        }
        ?>
